...
**Adopt the tokens as-is.** Import `packages/ame-tokens/tokens.css` and bind the custom properties. You inherit a system in which every rendered contrast pair is measured in both themes. One thing to know before you bind anything: the tokens are emitted under a `.portfolio-root` class. That class must sit on an ancestor of anything binding them, with `[data-theme="dark"]` under it to re-point the themed aliases. The name lags the extraction, and renaming it would break consumers, so it is written down rather than quietly changed.
