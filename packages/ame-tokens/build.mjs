#!/usr/bin/env node
/*
  Token build. Reads the DTCG 2025.10 token files under base/, semantic/, and
 ...
 * from its closest typed group, or taken from the token it references. DTCG
...
// ── CSS serialization, one function per DTCG type ───────────────────────────
...
/**
 * The one derived property. DTCG has no asset type, so the grain cannot be a
...
/*
  ...
  Source of truth: packages/ame-tokens/{base,semantic,component}/*.json (DTCG 2025.10)
...
// ── Emit ────────────────────────────────────────────────────────────────────
// The one committed home. There is no second copy to drift from: the portfolio
// and Metis bind this file through the package boundary, and check.mjs (B4)
// rebuilds it from source to prove the committed bytes still match.
function main() {
  const { tokens } = buildTokens()
  writeFileSync(join(ROOT, 'tokens.css'), renderCss(tokens))
  console.log(
