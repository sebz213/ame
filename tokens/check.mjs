...
const violations = []
const drift = {}
// Assets the K1 budget excused, with what each one measured. Reported, and counted
// in the verdict, so "green" is never quietly "green with exceptions".
const waivedAssets = []
...
;(function checkAssetBudget() {
  ...
  /*
    A POINTER IS A MEASUREMENT, NOT A FILE.

    Git LFS replaces a tracked file with a ~130-byte text stub that states the real
    object's size. Weighing the stub told K1 that an 83MB model was 130 bytes and
    comfortably under budget — true about the bytes present, useless as the guarantee
    the clause exists to give, and it made the verdict depend on whether the checkout
    happened to smudge. The same tree passed or failed on a git config.

    So when the bytes on disk are a pointer, the size comes from the pointer. K1 is
    then byte-exact on any checkout, smudged or not, which is what lets CI stop
    fetching LFS objects at all (lfs: false in both workflows) and still measure the
    real thing.

    Only files small enough to BE a pointer are opened, so this reads a few hundred
    bytes and never the 83MB it is protecting against. A stub that announces itself
    and then does not state a size is a hard failure rather than a silent fall back to
  ...
  */
  ...
  const measure = (f, onDisk) => {
    ...
    let head
    try {
      head = readFileSync(join(REPO, f), 'utf8')
    } catch {
      return onDisk
    }
    if (!head.startsWith(POINTER_PREFIX)) return onDisk
    const m = head.match(/^size (\d+)$/m)
    if (!m) {
      fail(a.id, `${f} is an LFS pointer with no size line, so its real byte count cannot be read. K1 would otherwise weigh the ${onDisk} B stub and pass a file of unknown size.`)
      return onDisk
    }
    return Number(m[1])
  }
  for (const f of walkFiles(a.root)) {
    ...
    */
    ...
    if (w !== undefined) {
      /*
        TWO SHAPES. A plain number is a legacy waiver: a ceiling and nothing else.
        An object states the whole case -- clause, ceiling, bytes, owner, date,
        reduction target, the reduction, and the trigger that ends it. Both ratchet
        the same way; only one can be read without asking someone.
      */
      const ceiling = typeof w === 'number' ? w : w.bytes
      if (size > ceiling)
        fail(a.id, `${f} is ${size} B, above its waived ceiling of ${ceiling} B. A waiver only ratchets down; shrink it in its reduction order, never grow it.`)
      /*
        RECORDED, NOT SWALLOWED. A waiver that produces no output is a bypass wearing
        a waiver's name: the run goes green and nothing on the page says what the
        check excused. Every waived asset is reported below and counted in the
        verdict, so a clean run reads PASS+2W and a reader is told the number without
        having to go looking for it.
      */
      waivedAssets.push({ file: f, size, ceiling, budget: a.budgets[cls], spec: typeof w === 'number' ? null : w })
...
;(function checkWorkflows() {
  ...
  for (const f of readdirSync(dir)) {
    ...
    const src = read(rel)
    /*
      Split on the newline and DROP THE CARRIAGE RETURN.

      The `run:` patterns below end in `$`, and \r is a line terminator in JS
      regex: on a CRLF checkout every one of them matched nothing. W1 stopped
      verifying that CI names scripts that exist, and W5 reported every script
      as uninvoked. The clause did not fail — it went quiet, which is worse,
      and it went quiet on exactly the platform a contributor is most likely to
      clone from. Linux CI never saw it; a fresh Windows clone sees it every
...
;(function checkDenominator() {
  ...
  /*
    ...
    It was a hand-kept allowlist naming app, content and hooks — the portfolio
    surfaces a package checkout does not carry. Each entry was true, and the list was
    still the wrong shape: package_manifest already states exactly what travels, so
    the allowlist was a second copy of that fact, maintained by memory, that would go
    stale the first time the manifest changed and nobody remembered this list existed.
    Z4 is the clause that objects to lists like it — they may be emptied, never
    extended — and the honest way to empty this one was to stop needing it.

    So: in PACKAGE scope, a scan root the manifest does not carry is absent by design
    and its clause did not run rather than passing over nothing. In every other scope
    every scan root must exist, which is the guarantee Z3 is for — the derivation
    narrows the excuse to the one situation that earns it instead of trusting a name.

    A prefix match, because a manifest entry can be a directory: packages/ame-tokens
    carries packages/ame-tokens/base, and a scan of the latter is a scan of something
    that travels.
  */
  const carried = (rel) =>
    RULES.package_manifest.paths.some((m) => rel === m || rel.startsWith(m + '/'))
  const absentByDesign = (rel) => SCOPE === 'package' && !carried(rel)
  for (const [rel, existed] of scanRoots)
    if (!existed && !absentByDesign(rel))
      fail(
        d.id,
        `a clause scanned ${rel}, which does not exist; the scan reads zero files and its clause reports zero findings over an empty set. Point it at a real path, or drop it from the list. A path that is legitimately absent in package scope is one package_manifest does not carry, and that is derived here rather than named — so add it to the manifest if it should travel, and if it should not, the scan should not be reaching for it in this scope.`,
      )
})()
...
// ── LC1. One licence, and every declaration of it agrees ─────────────────────
// The SPDX id and the files that must state it are data in invariants.json >
// licence; nothing here restates either. A relicence has to touch every file in
// that list or this fails, which is the only reason the list is worth keeping.
//
// Written the day MIT became Apache-2.0, because the identifier was already in
// eight places and getting seven of them right is indistinguishable from getting
// all eight right until someone reads the eighth. That is the README-figures
// failure exactly (264 tokens over a tree holding 339), and a licence is a worse
// thing to be wrong about than a token count.
;(function checkLicence() {
  if (!clauseInScope('LC1')) return
  const L = RULES.licence
  /*
    The same file, two names. docs/package/README.md is the package's front door
    written here, and the extraction publishes it at the root as README.md
    (package_manifest.paratext). A declaration has to be found wherever the tree
    it is running in keeps it, or L1 fails in the package for a file that is
    present under its published name -- which is how this was found, on the
    first extraction after the clause was written.
  */
  const paratext = RULES.package_manifest?.paratext ?? {}
  const resolve = (p) => {
    if (existsSync(join(REPO, p))) return join(REPO, p)
    const published = paratext[p]
    if (published && existsSync(join(REPO, published))) return join(REPO, published)
    return null
  }
  for (const d of L.declarations) {
    const f = resolve(d.file)
    if (!f) {
      fail(L.id, `${d.file} declares the licence and is not in the tree under that name or its published one. Either it comes back or it leaves invariants.json > licence.declarations.`)
      continue
    }
    const src = readFileSync(f, 'utf8')
    if (!src.includes(d.must_match))
      fail(L.id, `${d.file} does not state ${JSON.stringify(d.must_match)}. The licence is ${L.spdx}; a declaration that disagrees with it is the drift this clause exists to catch.`)
    /*
      A stale identifier left behind is the real failure mode, not an absent one.
      Only the declaring files are read: CHANGELOG, DECISIONS.md, docs/orders and
      the dated preflight correctly still say MIT, because they are records of
      what was true then and rewriting history to match a new licence would be a
      worse lie than the drift.
    */
    for (const bad of L.forbidden) {
      const stale = new RegExp(`licen[cs]e\\b[^\\n]{0,40}\\b${bad}\\b|\\b${bad}\\b[^\\n]{0,20}licen[cs]e`, 'i')
      /*
        A PARAGRAPH naming a declared third-party holder is exempt. NOTICE has
        to be able to say the icon set is MIT, Copyright Schweizerische
        Eidgenossenschaft, without that reading as a stale claim about this
        project -- and rewriting the prose to satisfy the checker would be
        dodging it rather than fixing it.

        Paragraph, not line, because prose wraps: this clause first fired on
        exactly that, with "MIT licensed, Copyright (c)" ending one line and the
        holder starting the next. A rule whose verdict depends on where a line
        break lands is measuring the typography, not the claim.

        Still narrow -- the holder must appear in the SAME paragraph. A bare
        "MIT" in a paragraph of its own is a leftover and still fails.
      */
      const holders = (L.third_party ?? []).map((t) => t.holder)
      const offending = src
        .split(/\r?\n\s*\r?\n/)
        .filter((para) => stale.test(para))
        .filter((para) => !holders.some((h) => para.includes(h)))
      if (offending.length)
        fail(L.id, `${d.file} still names ${bad} beside the word licence: ${JSON.stringify(offending[0].trim().replace(/\s+/g, ' ').slice(0, 70))}. The tree is ${L.spdx} now; this is a leftover.`)
    }
  }
  // LICENSE-DOCS is the one home for which documents carry the second offer, so
  // a file named there that does not exist would make the offer unusable.
  for (const f of L.docs_covered)
    if (!existsSync(join(REPO, f)))
      fail(L.id, `licence.docs_covered names ${f}, which is not in the tree. The CC BY offer would point at nothing.`)

  /*
    Third-party notices are a CONDITION, not paperwork. MIT permits the icon set
    to be used here, including commercially and inside an Apache-2.0 tree, on the
    single condition that its permission notice is included in all copies or
    substantial portions. 218 glyphs is a substantial portion, so the notice is
    the term that makes the use lawful.

    A condition that depends on nobody deleting a file is not a condition, it is
    a hope. This reads the text on every gate run.
  */
  for (const t of L.third_party ?? []) {
    const nf = join(REPO, t.notice_file)
    if (!existsSync(nf)) {
      fail(L.id, `${t.notice_file} is missing. ${t.what} is ${t.spdx} and its notice must travel with the copies; without that file the redistribution does not meet its terms.`)
      continue
    }
    const notice = readFileSync(nf, 'utf8')
    for (const required of t.must_contain)
      if (!notice.includes(required))
        fail(L.id, `${t.notice_file} no longer contains ${JSON.stringify(required.slice(0, 48))}. That text is a condition of the ${t.spdx} grant covering ${t.what}, not decoration.`)
  }
})()
...
/*
  PASS+2W, not PASS. A run that excused something says so in the one line that gets
  committed to runs.log, because that line is what a reader skims months later and a
  bare PASS would let two 500KB images become invisible by habit.
*/
const verdict =
  violations.length || grew ? 'FAIL' : waivedAssets.length ? `PASS+${waivedAssets.length}W` : 'PASS'
...
if (waivedAssets.length) {
  ...
  for (const w of waivedAssets) {
    ...
    if (w.spec) {
      ...
      console.log('      legacy waiver: byte ceiling only, held under asset_budget.waived_because')
    }
  }
}
