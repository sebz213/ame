...
workflow with patterns ending in `$`; the parser split on `\n` and left the
`\r`, and `\r` is a line terminator in JS regex, so on a CRLF checkout `$` never
matched and both clauses parsed nothing at all. W5 concluded nothing was ever
invoked and fired on everything. W1, which verifies that CI names scripts that
exist, simply stopped verifying — and printed no different output while doing
it.
...
while checking nothing. It is the same shape as B4's CRLF failure, which is what
makes it a pattern rather than an incident: reasoning about text cleared the
tree twice, and a clone on a different machine refused it in one run both times.
...
quiet the same way. The honest general answer is that clauses need
disconfirming fixtures the way the gate now has one — a case each clause must
fail on — so that a clause which stops asking gets caught by the thing it stops
catching. `examples/` (D-82) is that instrument for the gate as a whole; it does
not yet exist per clause. Recorded as open, not solved.
...
The sync branch had been built as an orphan commit: a fresh `git init` over the
extracted tree. It passed CI. It also had no common ancestor with `main`, so it
...
command and immediately showed two deletions — the extraction preflight and the
decision-status classification, both cited by `docs/PROVENANCE.md` as the
evidence for its claims. The sync would have removed the documents and left the
...
Two things generalise. Preferring the orphan commit was choosing a clean-looking
result over a legible one, and the illegibility was load-bearing: the check that
...
**The prose gets a second offer.** `tokens/contract.md`, `tokens/decisions.md`,
`STANDARD.md` and `docs/LEXICON.md` are the documents carrying the reasoning
rather than the mechanism, and they are the part most likely to be quoted in a
post, a paper or a talk. A software licence answers the wrong question about
prose, so those four are offered under either Apache-2.0 or CC BY 4.0, at the
reader's choice, with the file list living in `LICENSE-DOCS` and nowhere else.
Nothing is withheld by the addition; it exists to remove an argument.

**Clean to do, and only because nothing was published.** `git log` shows one
human author across 623 commits; the dependabot bumps and the assistant commits
carry no independent copyright claim. A relicence with outside contributors
needs their consent, and this one had no one to ask. It is also free of the
usual catch — an MIT release already in someone's hands stays MIT forever — for
the plain reason that the repository has always been private. Doing this after
publication would have been a different and much worse decision.

**What it cost, and the thing worth keeping.** The identifier turned out to live
in eight places. Seven were updated by hand and the eighth, the package
README's "MIT, stated in LICENSE", survived — the front door of the published
package, contradicting the licence beside it. That is the README-figures failure
exactly (264 tokens over a tree holding 339), in a place where being wrong costs
more than a stale count. So the licence became a checked invariant: clause L1
reads every declaring file on every gate run, and it failed on its first run
against the tree I had already "finished". Dated records are deliberately not
read, because a CHANGELOG that says MIT is correct about the day it describes
and rewriting it would be the worse lie.
...
listed in `VENDORED.md`, every listed path exists, and a component cannot be
added without recording it. It did not catch 218 third-party files, because it
governs *declared roots* and `components/ame/icons` was never declared one. The
clause was working perfectly, on a set that did not include the thing. A rule
scoped by a list has the list's blind spots, and nothing measures what a list
omits — the same shape as D-84's manifest, where omission means both *not mine*
and *forgotten*, three entries apart in this same log.

What actually surfaced it was asking what a licence asserts. Apache-2.0 says the
licensor may license what is in the tree; that assertion is only as good as the
provenance under it, and reading the tree for provenance found none for the
...
**L1 got a second lesson in the same session.** Its stale-identifier scan fired
on `NOTICE` legitimately saying the icon set is MIT — a true statement about
someone else's work, read as a stale claim about this project. The fix was not
to rewrite the sentence until the checker was happy, which is dodging a linter
rather than fixing it; a paragraph naming a declared third-party holder is now
exempt. Paragraph and not line because the first attempt failed on where a line
break happened to fall, and a rule whose verdict depends on typography is
...
contract clause maps to an invariant and a check, and every check is claimed by
a clause. It passed because both entries were structurally valid — a real
invariant, a real check function — and because a duplicate key in an object is
not a mismatch, it is one key. `contract.md` carried two clauses both labelled
`L1` and nothing compared them. Z1 verifies that the mapping *resolves*, never
that the identifiers are *distinct*.
...
collision cost nothing permanent because it was caught in the same session, but
it was two commits from being pushed, and a layering clause silently missing
its census entry is the kind of thing that stays wrong for months.

**The pattern is now three deep and worth naming.** D-52 and D-54 collided with
existing decision numbers on the way in. D-80 was written when D-80 existed.
Now two clauses. Every one of them is an identifier assigned by hand into a
namespace nothing enforces uniqueness over, and every one was caught by a human
