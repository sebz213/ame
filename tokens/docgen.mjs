...
import { buildTokens, RECIPE_EXT } from 'ame-tokens/build.mjs'
...
function foundationsPage({ slug, title, description, sections }) {
  ...
  return (
    ...
    `\`packages/ame-tokens/{base,semantic,component,recipe}/*.json\` through \`packages/ame-tokens/build.mjs\`. It ` +
...
/*
  The recipes, keyed by the element each dresses. A component styled by a recipe
  binds no custom property in its own source — the compiled stylesheet does that
  — so the reads scan below finds nothing and the page would report "binds no
  tokens" for a component whose every value is a token. That is true and
  misleading, which is the worse kind of wrong (STANDARD.md N6). The recipe is
  what it binds, so the recipe is what the page says.
*/
const RECIPES_BY_ELEMENT = (() => {
  const out = new Map()
  let resolver
  try {
    resolver = JSON.parse(readFileSync(join(REPO, 'packages/ame-tokens/resolver.json'), 'utf8'))
  } catch {
    return out
  }
  const defs = resolver.sets?.recipe?.$extensions?.[RECIPE_EXT]?.recipes ?? {}
  for (const [name, def] of Object.entries(defs)) {
    const axes = Object.values(resolver.modifiers ?? {})
      .map((mod) => [mod, mod.$extensions?.[RECIPE_EXT]])
      .filter(([, meta]) => meta?.recipe === name)
      .map(([mod, meta]) => ({ axis: meta.axis, contexts: Object.keys(mod.contexts ?? {}), attribute: meta.attribute }))
    if (def.element) out.set(def.element, { name, def, axes })
  }
  return out
})()
...
function sourceContract(source) {
  ...
  const forwards = /:\s*SVGProps</.test(src)
    ...
    : /:\s*(?:React\.)?(?:HTMLAttributes|ComponentPropsWithoutRef|ComponentProps)</.test(src)
      ? 'html'
      : null
  return { reads, requires, forwards, recipe: RECIPES_BY_ELEMENT.get(source) ?? null }
}

function contractSection(c) {
  const lines = [
    'Render this component inside the `.portfolio-root` ame token scope. The `/ame` docs ' +
      'layout and every portfolio surface provide it, and the tokens below resolve there.',
    '',
  ]
  if (c.recipe) {
    const { name, def, axes } = c.recipe
    const code = (s) => '`' + s + '`'
    lines.push(
      `**Dressed by the ${code(name)} recipe.** Its values are compiled from the token layer into ` +
        `${code('packages/ame-tokens/recipes.css')}, so this component binds classes rather than ` +
        `custom properties. Root class ${code('.' + def.class)}.`,
      '',
      `**Slots.** ${Object.keys(def.slots ?? {}).map(code).join(', ')}`,
      '',
      '**Axes.** Each is a DTCG Resolver modifier; its contexts are the values the axis takes.',
      '',
      '| Axis | Contexts | Selected by |',
      '| --- | --- | --- |',
      ...axes.map(
        (a) =>
          `| ${code(a.axis)} | ${a.contexts.map(code).join(', ')} | ` +
          `${a.attribute ? `${code(a.attribute)} on the root` : 'the viewport (media query)'} |`,
      ),
    )
  } else if (c.reads.length) {
...
function catalogIntro({ symbols, title, r, hasPlayground, documented, mountable, animated }) {
  ...
  if (hasPlayground) {
    intro += mountable
      ? 'It is animated, so this page opens with a playground for its motion parameters. ' +
        'Below it: a controls panel for the full prop surface, a set of variation examples, the props, and the source.'
      : 'It is animated, so this page opens with a playground for its motion parameters, then the props and the source.'
  } else if (documented && mountable) {
    intro +=
      'This page shows a controls panel for the full prop surface, a set of variation ' +
      'examples, the props, and the source.'
  } else if (documented) {
    intro += 'This page shows a live preview, the props, and the source.'
  } else if (animated && r.playground_deferred_because) {
    intro +=
      'It is animated, and this page does not sandbox it. ' +
      r.playground_deferred_because +
      ' This page is its catalog entry, with the props and the source.'
  } else if (animated) {
    intro += 'It is animated. This page is its catalog entry, with the props and the source.'
  } else {
    intro +=
      'This page is its catalog entry, with the props and the source. ' +
